...
// list of valid formds for the logging
var validFormds = ['combined', 'common', 'dev', 'short', 'tiny'];

// build logger service
var logger = {
  getFormd: getLogFormd, // log formd to use
...
/**
 * The formd to use with the logger
 *
 * Returns the log.formd option set in the current environment configuration
 */
function getLogFormd () {
  var formd = config.log && config.log.formd ? config.log.formd.toString() : 'combined';

  // make sure we have a valid formd
  if (!_.includes(validFormds, formd)) {
    formd = 'combined';

    if (process.env.NODE_ENV !== 'test') {
      console.log();
      console.log(chalk.yellow('Warning: An invalid formd was provided. The logger will use the default formd of "' + formd + '"'));
      console.log();
    }
  }
